...
L08b pilots reliable autosave for exactly one existing editor:
`src/components/athlete/WeekDetail.tsx`. Its weekly-review textarea writes one
row in `athlete_week_notes`.

The calendar editor still calls the legacy `updateWeekNote` in `src/app/page.tsx`.
It is deliberately outside the pilot. No group, session, feedback, goal,
statistics, invitation, deletion, table, RLS policy, RPC, migration or remote
configuration changes in L08b.
...
With the flag absent, invalid, disabled, or outside local development,
`WeekDetail` uses its unchanged legacy upsert path. A single edit chooses
either legacy or V2; it never dual-writes.
...
In the local pilot only, the textarea can show `Enregistrement...`,
...
1. Keep the flag absent and start `npm run dev`; edit the weekly-review note.
...
3. Restart the development server. In the weekly-review editor only, test a
   slow edit, rapid typing, blur, a forced offline error, manual retry, changing
   athlete or week during a save, and a reload after a confirmed save.
