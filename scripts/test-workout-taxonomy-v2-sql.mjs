...
const containerName = `myrideplan-l15d-${process.pid}`;
const files = [
  ...
  "supabase/tests/workout-taxonomy-v2.sql",
];
const coachUserId = "00000000-0000-0000-0000-000000000171";
const categoryId = "71000000-0000-0000-0000-000000000174";
...
try {
  run("docker", [
    ...
    "-lc", "initdb -D /tmp/l15d-postgres >/dev/null && pg_ctl -D /tmp/l15d-postgres -o \"-c listen_addresses=''\" -w start >/dev/null && tail -f /dev/null",
  ]);
  ...
  const first = executeSqlAsync(`
    ...
    select public.rename_workout_category_v2('${categoryId}', 'Concurrence A', null);
    select pg_sleep(0.4);
    commit;
  `);
  ...
  const second = await executeSqlAsync(`
    begin;
    select set_config('request.jwt.claim.sub', '${coachUserId}', false);
    select public.rename_workout_category_v2('${categoryId}', 'Concurrence B', null);
    commit;
  `);
  const firstResult = await first;
  if (firstResult.status !== 0 || second.status !== 0) {
    throw new Error(`Concurrent taxonomy rename failed\n${firstResult.stderr}${second.stderr}`);
  }

  const proof = run(
    "docker",
    ["exec", "-i", "--user", "postgres", containerName, "psql", "-At", "--set", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"],
    `select json_build_object('taxonomy', (select name from public.workout_categories where id = '${categoryId}'), 'workouts', (select count(*) from public.workout_library where category = 'Concurrence B'));`,
  ).trim();
  if (proof !== '{"taxonomy" : "Concurrence B", "workouts" : 1}') {
    throw new Error(`Concurrent taxonomy rename left an unexpected state: ${proof}`);
  }

  const firstDelete = executeSqlAsync(`
    begin;
    select set_config('request.jwt.claim.sub', '${coachUserId}', false);
    select public.delete_workout_category_v2('Concurrence B');
    select pg_sleep(0.4);
    commit;
  `);
  await new Promise((resolve) => setTimeout(resolve, 75));
  const secondDelete = await executeSqlAsync(`
    begin;
    select set_config('request.jwt.claim.sub', '${coachUserId}', false);
    select public.delete_workout_category_v2('Concurrence B');
    commit;
  `);
  const firstDeleteResult = await firstDelete;
  if (firstDeleteResult.status !== 0 || secondDelete.status !== 0) {
    throw new Error(`Concurrent taxonomy deletion failed\n${firstDeleteResult.stderr}${secondDelete.stderr}`);
  }

  const deleteProof = run(
    "docker",
    ["exec", "-i", "--user", "postgres", containerName, "psql", "-At", "--set", "ON_ERROR_STOP=1", "-U", "postgres", "-d", "postgres"],
    "select (not exists (select 1 from public.workout_categories where name = 'Concurrence B') and not exists (select 1 from public.workout_library where category = 'Concurrence B'))::text;",
  ).trim();
  if (deleteProof !== "true") {
    throw new Error("Concurrent taxonomy deletion left a partial legacy state.");
  }

  console.log("Workout taxonomy V2 SQL migration test passed.");
} finally {
